...
/** Buscar productores por nombre comercial */
export async function searchProducersByName(query: string): Promise<PublicProducerProfile[]> {
  const res = await fetch(`${BASE_URL}/producer-profiles/search?q=${encodeURIComponent(query)}`);
  return handleResponse<PublicProducerProfile[]>(res);
}

/** Obtener productores recomendados */
export async function getRecommendedProducers(limit = 6): Promise<PublicProducerProfile[]> {
  const res = await fetch(`${BASE_URL}/producer-profiles/recommended?limit=${limit}`);
  return handleResponse<PublicProducerProfile[]>(res);
}
